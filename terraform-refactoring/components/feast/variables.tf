...
variable "ingress" {
  description = "Ingress provider for feast-ui from istio-pilot:ingress (supports same-model endpoint or cross-model offer)"
  type = object({
    kind     = string
    name     = optional(string, null)
    endpoint = optional(string, null)
...
variable "service_mesh" {
  description = "Service mesh provider for feast-ui from istio-beacon-k8s:service-mesh (ambient; supports same-model endpoint or cross-model offer)"
  type = object({
    kind     = string
    name     = optional(string, null)
    endpoint = optional(string, null)
