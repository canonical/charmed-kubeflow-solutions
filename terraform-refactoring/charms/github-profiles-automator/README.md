...
Single-charm module for `github-profiles-automator`.

Syncs Kubeflow `Profiles` / `RoleBindings` / `AuthorizationPolicies` from a
Profile Management Representation (PMR) file in a GitHub repository.

- `service-mesh` (requires) → `istio-beacon-k8s:service-mesh` (in-model, wired
  by the kubeflow product).
...
| Name | Type |
| ---- | ---- |
| [juju_application.github_profiles_automator](https://registry.terraform.io/providers/juju/juju/latest/docs/resources/application) | resource |
