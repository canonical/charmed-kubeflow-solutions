...
Single-charm module for the ambient Istio control plane (`istio-k8s`).

Deployed on its own into the `istio-system` model by the top-level deployment.
The deployment exposes the `istio-ingress-config` endpoint cross-model (via a
`juju_offer`) so the `istio-ingress-k8s` gateways in the `kubeflow` model can
attach to this control plane.
...
| ---- | ----------- | ---- | ------- | :------: |
| <a name="input_app_name"></a> [app\_name](#input\_app\_name) | Name to give the deployed application. | `string` | `"istio-k8s"` | no |
