...
Single-charm module for the ambient mesh waypoint/registration charm
(`istio-beacon-k8s`).

Stays in the `kubeflow` model alongside the two ingress gateways. It joins the
mesh via the in-cluster Istio control plane installed by `istio-k8s` (in the
`istio-system` model) and therefore needs **no** Juju relation to `istio-k8s`.

- `service-mesh` (provides) → on-mesh Kubeflow applications (in-model).
...
| ---- | ----------- | ---- | ------- | :------: |
| <a name="input_app_name"></a> [app\_name](#input\_app\_name) | Name to give the deployed application. | `string` | `"istio-beacon-k8s"` | no |
