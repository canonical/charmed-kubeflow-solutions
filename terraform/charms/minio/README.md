...
| ---- | ----------- | ---- | ------- | :------: |
| <a name="input_app_name"></a> [app\_name](#input\_app\_name) | Name to give the deployed application. | `string` | `"minio"` | no |
| <a name="input_base"></a> [base](#input\_base) | The operating system on which to deploy. | `string` | `"ubuntu@24.04"` | no |
| <a name="input_channel"></a> [channel](#input\_channel) | Channel of the charm. | `string` | `"1.10/stable"` | no |
| <a name="input_config"></a> [config](#input\_config) | Map for configuration options. | <pre>object({<br/>    access-key = optional(string)<br/>    secret-key = optional(string)<br/>  })</pre> | `{}` | no |
...
| <a name="input_storage_directives"></a> [storage\_directives](#input\_storage\_directives) | Storage directives | `map(string)` | <pre>{<br/>  "minio-data": "10G"<br/>}</pre> | no |
