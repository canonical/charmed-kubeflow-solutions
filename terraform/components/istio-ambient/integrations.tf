...
# wired here.
#
# Only the UI gateway is wired to istio-ingress-config: that relation exchanges
# ext-authz (forward-auth) details, which only the UI gateway uses. The M2M
# gateway is JWT-only (istio-request-auth) with no forward-auth, so it is
